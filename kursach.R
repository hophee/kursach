...
get_pacient_spyro <- function(df, pacient_code, roll_mean_num, long=F, safe=F) {
  ...
  spyro <- spyro %>% mutate(across(rn_r50_om:lf_phi5_grad, ~ fourie_filter(.)))  #применение фильтра Баттерворта
  spyro <- spyro %>% mutate(across(rn_r50_om:lf_phi5_grad, ~ scale(.))) # нормирование значений
  spyro <- spyro %>% mutate(across(rn_r50_om:lf_phi5_grad, ~ rollmean(., k = roll_mean_num, fill = NA))) # скользящее среднее
  if (long==F) {
...
get_pacient_spyro(spiro_data, my_patients[1], 7, safe=T)
get_pacient_spyro(spiro_data, my_patients[1], 7, long=T)

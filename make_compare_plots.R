...
p <- ggplot(data=df, aes(x=vrema_obsled)) +
  geom_line(aes(y=before_r50_om, color = 'Before')) +
  geom_line(aes(y=after_r50_om, color = 'After')) +
  scale_color_manual(values = c("Before" = '#ef233c', "After" = '#3a86ff'),
                     name = "Измерения",
                     labels = c("До фильтрации", "После фильтрации")) +
  labs(x='Время',
       y='Объём лёгких',
       title='Сравнение данных до и после', 
       subtitle = 'Спирограмма, пациент П11-3')
p
...
p <- ggplot(data=df) +
  ...
  labs(x='Объём лёгких',
       y='Объёмная скорость',
       title='Сравнение данных до и после', 
       subtitle = 'Пневмотахограмма, пациент П11-3')
p
ggsave('p11_3_pnevmo_compare.png', plot = p, width = 10, height = 6, dpi=600)